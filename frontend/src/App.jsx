...
function App() {
  ...
  const [history, setHistory] = useState({}); // Stores graph history

  // Fetch realtime data every 1 second
  useEffect(() => {
  ...
  const updateLimits = (device) => {
    ...
    fetch("http://127.0.0.1:8000/set_limits", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        device: device,
  ...
  return (
    ...
    textAlign: "center",
    marginBottom: "30px",
    color: "#1e90ff"      // ← your new color (example: DodgerBlue)
}}>
  🌐 Smart IoT Dashboard
</h1>
